...
p17 <- ggplot(titv, aes(fill=Mutation, y=`%`, x=Method)) + 
  ...
  scale_x_discrete(limits=c("epPCR (Mutazyme II)", "epPCR (Taq, Mn2+)", "EvolVR", "MutaT7", "PACE", "OrthoRep", "TRIDENT", "ICE", "YeastIT1", "YeastIT2"))
...
p18 <- ggplot(sixtypes, aes(fill=factor(`Mutation type`, levels=c("AT>GC","GC>AT","AT>CG", "AT>TA", "GC>TA", "GC>CG")), y=`%`, x=Method)) + 
  ...
  theme(legend.title = element_blank()) +
  theme(plot.background = element_rect(fill = "transparent",colour = NA),  legend.box.background = element_rect(fill = "transparent", colour=NA)) 
  p18 <- p18 + scale_x_discrete(limits=c("epPCR (Mutazyme II)", "epPCR (Taq, Mn2+)", "EvolVR", "MutaT7", "PACE", "OrthoRep", "TRIDENT", "ICE", "YeastIT", "YeastIT2"))
...
p19 <- p19 + xlab("") + ylab("Mutation rate") 
...
p19 <- p19 + scale_x_discrete(limits=c("epPCR (Mutazyme II)", "epPCR (Taq, Mn2+)", "EvolVR", "MutaT7", "PACE", "OrthoRep", "TRIDENT", "ICE", "YeastIT1", "YeastIT2"))
p19 <- p19 + theme(axis.text.x = element_text(angle = 50, hjust = 1)) 
